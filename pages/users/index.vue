...
<template>
  ...
  <div class="bg-white bg-opacity-90 backdrop-blur-md rounded-2xl rounded-2xl shadow-2xl overflow-hidden">
    <div class="space-y-2">
      <div v-for="(member, index) in members" :key="member.id" class="flex items-center justify-between border-b px-4 py-2">
        <div class="flex">
          <span class="pr-3 font-bold">  (TBU-{{ member[16] }}) </span>
          <span class="text-gray-800">  {{ member[1] }}</span>
        </div>
